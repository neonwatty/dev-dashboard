...
class DiscourseService
  attr_reader :source, :client, :source_identifier

  def initialize(source)
    @source = source
    @source_identifier = determine_source_identifier
    setup_client
  end
  ...
  def create_post_from_topic(topic_data)
    post = Post.find_or_initialize_by(
      ...
      external_id: topic_data['id'].to_s
    )
    ...
    if @source.config_hash['fetch_full_content'] == true
      enhance_with_full_content(post, topic_data['id'])
    end
    
    post.save!
    true
  rescue => e
    Rails.logger.error "Error creating post from topic #{topic_data['id']}: #{e.message}"
    false
  end
