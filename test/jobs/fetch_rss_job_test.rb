...
class FetchRssJobTest < ActiveJob::TestCase
  ...
  test "should handle HTTP redirects" do
    # Mock RSS content
    rss_content = <<~RSS
      <?xml version="1.0" encoding="UTF-8"?>
      <rss version="2.0">
        <channel>
          ...
          <item>
            <title>Ruby Article After Redirect</title>
            <link>https://example.com/redirected-article</link>
            <description>Article accessed after redirect</description>
            <pubDate>Tue, 09 Jul 2025 12:00:00 +0000</pubDate>
          </item>
        </channel>
      </rss>
    ...
    # Mock redirect from original URL to new URL
    new_url = 'https://newlocation.example.com/rss'
    stub_request(:get, @source.url)
    ...
    # Should create post after following redirect
    assert_difference('Post.count', 1) do
      FetchRssJob.perform_now(@source.id)
    end

    # Verify source status was updated successfully
    @source.reload
    assert_includes @source.status, 'ok'
    
    # Verify post was created
    post = Post.find_by(source: @source.name, title: 'Ruby Article After Redirect')
    assert_not_nil post
  end
  ...
  test "should handle too many redirects" do
    ...
    stub_request(:get, @source.url)
      .to_return(status: 302, headers: { 'Location' => 'https://redirect1.com/rss' })
    stub_request(:get, 'https://redirect1.com/rss')
      .to_return(status: 302, headers: { 'Location' => 'https://redirect2.com/rss' })
    stub_request(:get, 'https://redirect2.com/rss')
      .to_return(status: 302, headers: { 'Location' => 'https://redirect3.com/rss' })
    stub_request(:get, 'https://redirect3.com/rss')
      .to_return(status: 302, headers: { 'Location' => 'https://redirect4.com/rss' })
    stub_request(:get, 'https://redirect4.com/rss')
      .to_return(status: 302, headers: { 'Location' => 'https://redirect5.com/rss' })
    stub_request(:get, 'https://redirect5.com/rss')
      .to_return(status: 302, headers: { 'Location' => 'https://redirect6.com/rss' })

    # Should not create any posts and report error
    assert_no_difference('Post.count') do
      FetchRssJob.perform_now(@source.id)
    end

    @source.reload
    assert_includes @source.status, 'error: Too many redirects'
  end
